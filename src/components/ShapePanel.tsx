  ...
  return <fieldset disabled={disabled}>
    ...
    {!view.shape.filled && <>
      ...
      <div className="shape-number-control">
        <NumberField label="边框粗细" value={view.shape.lineWidth} min={1} max={100} unit="px" cancelOnEscape onChange={lineWidth => engine.updateShape({ lineWidth })} />
    ...
    <div className="shape-number-control">
      <NumberField label="不透明度（%）" value={view.shape.opacity} min={0} max={100} cancelOnEscape onChange={opacity => engine.updateShape({ opacity })} />
    ...
    {view.shapeKind === "rect" && <div className="shape-number-control shape-radius-control">
      <NumberField label="矩形圆角" value={view.shape.radius} min={0} max={view.shapeRadiusMax ?? 500} unit="px" cancelOnEscape onChange={radius => engine.updateShape({ radius })} />
