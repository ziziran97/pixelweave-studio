...
try {
  ...
  const panel = host.querySelector<HTMLElement>(".panel-content")!;
  const warningVisible = () => {
    const warning = host.querySelector<HTMLElement>('.text-validation-error')!.getBoundingClientRect();
    const bounds = panel.getBoundingClientRect(), sticky = host.querySelector('.text-add-sticky')!.getBoundingClientRect();
    return warning.top >= Math.max(bounds.top, sticky.bottom) && warning.bottom <= bounds.bottom;
  };
  panel.scrollTop = panel.scrollHeight; await paint();
  const scrolled = panel.scrollTop;
  check(scrolled > 0 && !warningVisible(), "属性栏滚动后具体违禁词可处于视野外，覆盖定位恢复场景");
  ...
  check(panel.scrollTop === scrolled, "定位隐藏锁定图层时不滚动原选中文字的属性栏");
  ...
  check(warningVisible(), "定位当前已选问题图层也将具体违禁词完整带入视野且不被固定添加按钮遮挡");
  panel.scrollTop = panel.scrollHeight; await paint();
  const styleScroll = panel.scrollTop;
  button("加粗").click(); await paint();
  check(panel.scrollTop === styleScroll, "查看问题后调整文字样式不重复滚动属性栏");
  locate().click(); await paint();
  ...
  check(warningVisible(), "循环再次定位同一问题图层时重新显示具体违禁词");
  panel.scrollTop = panel.scrollHeight; await paint();
  button("收起工具属性").click(); await paint();
  locate().click(); await paint();
  check(host.querySelector<HTMLElement>("#tool-settings")!.hidden, "定位隐藏锁定问题仍保留左侧收起状态");
  locate().click(); await paint();
  check(!host.querySelector<HTMLElement>("#tool-settings")!.hidden && warningVisible(), "左侧收起后定位可编辑问题会展开并显示具体违禁词");
  locate().click(); await paint();
  rows[1].querySelector<HTMLButtonElement>('.layer-controls button')!.click(); await paint();
  ...
  rows[1].querySelector<HTMLButtonElement>('.layer-select')!.click(); await paint();
  panel.scrollTop = panel.scrollHeight; await paint();
  const ordinaryScroll = panel.scrollTop;
  rows[0].querySelector<HTMLButtonElement>('.layer-select')!.click(); await paint();
  check(panel.scrollTop === ordinaryScroll, "普通选择不重新执行之前的问题定位滚动请求");
  rows[1].querySelector<HTMLButtonElement>('.layer-select')!.click(); await paint();
