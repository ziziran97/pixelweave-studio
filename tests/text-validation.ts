...
export async function checkStandalonePreview(check: (value: boolean, message: string) => void) {
  ...
  try {
    ...
    check(demo.state().previewScenario === "success" && !!demo.state().textError?.includes("durable") && demo.state().canSubmit && !demo.state().saved && !demo.state().submissionProgress, "默认完整流程仍先拦截 durable，不进入后续模拟并保留草稿");
    edit(demo.editor, "A DURABLE, supreme item — durable SUPREME");
    await demo.confirm(() => demo.editor.submitReplacement());
    check(demo.state().textError === "包含违禁词：durable、supreme，请修改后再次替换。" && demo.state().problemObjectIds?.length === 1 && !demo.state().submissionProgress, "同一文案同时返回两个模拟违禁词，重复及大小写变体去重，问题数量按图层统计");
    edit(demo.editor, "A supreme item");
    await demo.confirm(() => demo.editor.submitReplacement());
    check(demo.state().textError === "包含违禁词：supreme，请修改后再次替换。" && demo.state().canSubmit && !demo.state().saved, "仅 supreme 也被拦截，复检不残留已移除的 durable");
    edit(demo.editor, "reliable");
    check(!demo.state().textError, "演示文案修改后清除旧问题，等待再次替换");
    await demo.confirm(() => demo.editor.submitReplacement());
    check(demo.state().submissionProgress?.status === "preview_complete" && !demo.state().closed && !demo.state().saved, "默认场景在文案通过后完成整套演示，仍未实际保存");
    demo.editor.finishPreviewSubmission();
    check(demo.state().notice.includes("未保存到任务") && !demo.state().submitting && demo.state().canSubmit, "完整演示返回后保留草稿并允许再次体验");
    check((demo.editor.canvas.getActiveObject() as Textbox).text === "reliable" && demo.state().canUndo, "演示检测保留文字、选择及可撤销编辑");
    demo.editor.setPreviewScenario("texts");
    edit(demo.editor, "nondurable durables nonsupreme supremes"); await demo.confirm(() => demo.editor.submitReplacement());
    check(demo.state().previewScenario === "texts" && !demo.state().textError && demo.state().notice.includes("文案检测通过") && !demo.state().submitting, "单独文案检测场景仍可选择，不把单词的一部分误判为 durable 或 supreme");
