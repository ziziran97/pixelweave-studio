...
export async function checkReplacementFlow(check: (condition: boolean, message: string) => void) {
  ...
  const { editor, state, click, dispose } = createEditor(adapter);
  ...
  try {
    ...
    blocked = false; await editor.submitReplacement();
    ...
    editor.setTool("draw"); await editor.undo(); await editor.submitReplacement();
    ...
    const second = editor.submitReplacement(); await settle(() => replaceCalls === 2);
    ...
    await editor.submitReplacement(); check(replaceCalls === 2, "保存成功后禁止再次提交");
  ...
  } finally { window.confirm = originalConfirm; dispose(); }

  let validations = 0, replacements = 0;
  const { editor: uploadEditor, state: uploadState, dispose: disposeUpload } = createEditor({
  ...
  try {
    ...
    await uploadEditor.submitReplacement();
  ...
  try {
    ...
    await local.editor.submitReplacement();
