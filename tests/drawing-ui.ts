...
try {
  ...
  const firstHintView = { width: viewport().clientWidth, height: viewport().clientHeight, zoom: zoom() };
  await drag(-180, -100, -80, -40);
  check(host.querySelector(".eraser-notice")?.textContent?.includes("按 V") === true && settings().textContent!.includes("右下角「选择」"), "首次绘制浮出选择提示，左侧保留选择方法");
  check(viewport().clientWidth === firstHintView.width && viewport().clientHeight === firstHintView.height && zoom() === firstHintView.zoom, "首次绘制提示不改变画布尺寸或缩放");
  await new Promise(resolve => setTimeout(resolve, 5200));
  check(!host.querySelector(".eraser-notice"), "首次绘制提醒约五秒自动收起");
  canvas().dispatchEvent(new KeyboardEvent("keydown", { key: "v", bubbles: true, cancelable: true })); await paint();
  check(button("选择").getAttribute("aria-pressed") === "true" && !host.querySelector(".layer-card.selected"), "按 V 退出连续绘制，不自动选中新对象");
  ...
  await select(rect);
  const keyView = zoom(); button("收起工具属性").click(); await paint();
  canvas().dispatchEvent(new KeyboardEvent("keydown", { key: "h", bubbles: true, cancelable: true })); await paint();
